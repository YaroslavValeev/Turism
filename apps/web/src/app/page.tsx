...
type Program = {
  ...
  cta: string | null;
  organizer?: { displayName: string; verificationStatus: string };
...
export default function HomePage() {
  ...
  const countryOptions = useMemo(() => {
    return [...new Set(allPrograms.map((program) => getProgramCountry(program).trim()).filter(Boolean))]
      .sort((left, right) => left.localeCompare(right, "ru"));
  }, [allPrograms]);

  const hasScopeFilters = Boolean(appliedDiscipline || appliedCountry);
  ...
  }, [allPrograms, appliedCountry, appliedDiscipline]);
  ...
  }, [catalogPrograms, levelFilter, dateFrom, dateTo]);
  ...
  const hotOfferSlides = useMemo<HotOfferSlide[]>(() => {
    ...
    return selected.map(({ program, index, daysUntilStart }) => {
      ...
      const facts = [
        `${formatHeroDate(program.startDate)} · ${program.durationDays} дн.`,
  ...
  return (
    <>
      ...
      >
        {hasScopeFilters && (
          <p style={{ color: "var(--mw-muted)", fontSize: "0.9rem", marginTop: 0 }}>
            Показаны программы по выбранным в шапке дисциплине и стране. Сбросьте фильтры, чтобы вернуться к полной подборке.
          </p>
        )}
        ...
        {!loading && filtered.length === 0 && catalogPrograms.length > 0 && (
          <p>Нет программ по выбранным уровню и датам. Сбросьте фильтры ниже или измените даты.</p>
        )}
        {!loading && catalogPrograms.length === 0 && allPrograms.length > 0 && (
          <p>
            Нет программ по выбранной дисциплине и стране. Сбросьте фильтры в шапке или выберите другую страну.
          </p>
        )}
