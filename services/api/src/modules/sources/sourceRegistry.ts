...
export async function upsertSourceByTypeAndHandle(db: PrismaClient, input: UpsertSourceInput) {
  ...
  return db.source.update({
    ...
    data: {
      ...
      importSessionId: input.importSessionId ?? existing.importSessionId,
      externalChannelId:
        input.externalChannelId === undefined ? undefined : input.externalChannelId,
      metaJson: {
        ...existingMeta,
        ...metaObject,
...
export async function pauseContractAutoSources(db: PrismaClient, organizerId: string) {
  return db.source.updateMany({
    where: {
      ...
      lifecycleState: { notIn: [SOURCE_LIFECYCLE.MANUAL_OVERRIDE, SOURCE_LIFECYCLE.ARCHIVED] },
    },
    data: {
      isActive: false,
