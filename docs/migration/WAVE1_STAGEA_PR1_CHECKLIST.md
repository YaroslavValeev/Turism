...
Статус: **выполнено в репозитории** (schema + DDL + `sourceRegistry` + unit-тесты на linkage).  
Принцип PR1: **маленький, обратимый, без policy-enforcement и без UI**.

Фактическая DDL-миграция: `services/api/prisma/migrations/20260503120000_source_external_channel_fk/migration.sql` (имя папки может отличаться от шаблона в п.2 — содержимое соответствует PR1: только `ADD COLUMN` + индекс + FK).  
Тесты: `services/api/src/modules/sources/sourceRegistry.upsert.test.ts`.
...
2. `services/api/prisma/migrations/20260503120000_source_external_channel_fk/migration.sql` (или эквивалентный timestamp; суть — одна DDL-only миграция PR1)
   - Только DDL:
     - add/adjust index,
